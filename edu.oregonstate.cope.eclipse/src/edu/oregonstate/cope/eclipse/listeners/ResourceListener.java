	...
	private void recordRefresh(IResourceDelta delta) {
		...
		if (type == IResource.FILE) {
			...
			try {
				...
				scanner.close();
				recorder.recordTextChange(contents, 0, 0, COPEPlugin.getDefault().getWorkspaceID() + "/" + affectedFile.getFullPath().toPortableString(), ClientRecorder.CHANGE_ORIGIN_REFRESH);
